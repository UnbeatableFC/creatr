...
export const getAnalytics = query({
  handler: async (ctx) => {
    ...
    const oldFollowers = await ctx.db
      .query("follows")
      .filter((q) => q.gt(q.field("createdAt") , thirtyDaysAgo))
      .collect();
    ...
    const commentsGrowth = totalComments > 0 ? (totalComments / oldComments.length) * 100 : 0; // Placeholder
    const followersGrowth = followersCount.length > 0 ? (followersCount.length / oldFollowers.length) * 100 : 0; // Placeholder

...
export default function DashboardPage() {
  ...
  const { data: analytics, isLoading: analyticsLoading } =
    useConvexQuery(api.dashboard.getAnalytics);
  const { data: recentPosts, isLoading: postsLoading } =
    useConvexQuery(api.dashboard.getPostsWithAnalytics, { limit: 5 });
  const { data: recentActivity, isLoading: activityLoading } =
    useConvexQuery(api.dashboard.getRecentActivity, { limit: 8 });
  const { data: dailyViewsData, isLoading: chartLoading } =
    useConvexQuery(api.dashboard.getDailyViews);
        ...
        <Card className="card-glass">
          ...
          <CardContent>
            ...
            {stats.commentsGrowth > 0 && (
              <div className="flex items-center text-xs text-green-400 mt-1">
                <TrendingUp className="h-3 w-3 mr-1" />+
                {stats.commentsGrowth}% from last month
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="card-glass">
          ...
          <CardContent>
            ...
            {stats.followersGrowth > 0 && (
              <div className="flex items-center text-xs text-green-400 mt-1">
                <TrendingUp className="h-3 w-3 mr-1" />+
                {stats.followersGrowth}% from last month
              </div>
            )}
          </CardContent>
        </Card>
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className="text-sm text-white">
                          ...
                          {activity.type === "like" &&
                            ` liked your post about "${activity.post}"`}
                          {activity.type === "comment" &&
